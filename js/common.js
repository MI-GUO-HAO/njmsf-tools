...
// ===== 联盟返佣（这就是"发链接赚佣金"的方式，不懂看这里）=====
// 淘宝客（阿里妈妈 Alimama）和京东联盟，都是平台官方的"推广赚返利"计划：
//   你把你专属的推广链接发出去 → 别人通过它下单 → 商家给你返利（买家价格不变）。
// 怎么拿到"带你自己 PID 的推广链接"？（各约 2 分钟，用你自己的淘宝/京东账号登录）
//   淘宝客：打开 https://pub.alimama.com → 登录淘宝 → 顶部"联盟产品"搜想推的商品
//           （如 补光灯 / 麦克风 / 素材会员）→ 点"立即推广"→ 选"导购推广"→ 复制出来的链接粘到下面 taobao。
//   京东联盟：打开 https://union.jd.com → 登录 → "商品推广"挑商品 → "获取推广链接" → 复制粘到下面 jd。
// 关键：链接里必须带你的 pid（形如 mm_123_456_789 或 unionId/positionId），否则佣金不算给你。
// 暂时不填也没事：推荐位会指向平台首页（不赚钱但站照常跑）。填了之后全站推荐位自动换成你的返佣链接。
